...
// New API: setPrecision(translation[m], rotation[rad])
  {
    moveit::core::CartesianPrecision prec;
    prec.translational = 0.002;
    prec.rotational    = 0.5 * M_PI / 180.0;
    cartesian->setPrecision(prec);
  }
  RCLCPP_INFO(logger, "Cartesian path planner set up with precision: translation=2mm, rotation=0.5deg");

  // cartesian->setJumpThreshold(2.0); 삭제부분
  // RCLCPP_INFO(logger, "Cartesian path planner set up with jump threshold: 2.0"); 삭제부분
